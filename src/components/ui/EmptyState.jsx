...
export default function EmptyState({ icon: Icon, title, description, actionText, onAction, className }) {
  return (
    <div className={cn("card-premium rounded-2xl p-12 lg:p-16 text-center border border-zinc-800/50", className)}>
      <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-2xl bg-zinc-800/50 mx-auto flex items-center justify-center mb-6">
        <Icon className="w-8 h-8 lg:w-10 lg:h-10 text-zinc-500" />
      </div>
      ...
      {actionText && onAction && (
        <Button onClick={onAction} className="brand-gradient text-white font-semibold shadow-lg shadow-orange-500/20">
          {actionText}
        </Button>
      )}
    </div>
