            ...
            <div className="h-80 relative overflow-visible" ref={chartContainerRef}>
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                              ...
                              <div key={`item-${index}`} className="flex items-center gap-2">
                                <div 
                                  ...
                                  style={{
                                    ...
                                    backgroundImage: entry.dataKey === 'baseline' ? 'repeating-linear-gradient(90deg, #71717a 0, #71717a 4px, transparent 4px, transparent 8px)' : 'none'
                                  }}
                                />
                                <span className="text-zinc-400">{entry.dataKey === 'baseline' ? 'Baseline' : selectedScenario.name}</span>
                              </div>
                            ))}
                          </div>
                          {/* Event markers row */}
                          <div className="flex flex-wrap justify-center gap-4 text-xs pt-2 border-t border-zinc-800/50">
                            {(baselineProjection?.yearByYear?.some(p => p.goalsFunded?.length > 0) || scenarioProjection?.yearByYear?.some(p => p.goalsFunded?.length > 0)) && (
                            ...
                            {(baselineProjection?.yearByYear?.some(p => p.debtPayoffs?.length > 0) || scenarioProjection?.yearByYear?.some(p => p.debtPayoffs?.length > 0)) && (
                              <div className="flex items-center gap-2">
                            ...
                            {(baselineProjection?.yearByYear?.some(p => p.liquidations?.some(l => l.type === 'release')) || scenarioProjection?.yearByYear?.some(p => p.liquidations?.some(l => l.type === 'release'))) && (
                              <div className="flex items-center gap-2">
                                <div className="w-6 h-0.5 bg-cyan-400" style={{backgroundImage: 'repeating-linear-gradient(90deg, #22d3ee 0, #22d3ee 4px, transparent 4px, transparent 8px)'}} />
                                <span className="text-zinc-400">Collateral Released</span>
                              </div>
                            )}
                            {(baselineProjection?.yearByYear?.some(p => p.loanPayoffs?.length > 0) || scenarioProjection?.yearByYear?.some(p => p.loanPayoffs?.length > 0)) && (
                              <div className="flex items-center gap-2">
                                <div className="w-6 h-0.5 bg-purple-400" style={{backgroundImage: 'repeating-linear-gradient(90deg, #c084fc 0, #c084fc 4px, transparent 4px, transparent 8px)'}} />
                                <span className="text-zinc-400">Loan Payoff / Equity Unlock</span>
                              </div>
                            )}
                          </div>
                        </div>
                  ...
                  {/* Baseline event lines (muted color) */}
                  {baselineProjection?.yearByYear?.filter(p => p.goalsFunded?.length > 0).map((p, idx) => (
                    <ReferenceLine
                      key={`baseline-goal-${p.age}-${idx}`}
                      x={p.age}
                      stroke="#60a5fa"
                      strokeDasharray="6 4"
                      strokeOpacity={0.3}
                  ...
                  {baselineProjection?.yearByYear?.filter(p => p.debtPayoffs?.length > 0).map((p, idx) => (
                    <ReferenceLine
                      key={`baseline-debt-${p.age}-${idx}`}
                      x={p.age}
                      stroke="#10b981"
                      strokeDasharray="5 5"
                      strokeOpacity={0.3}
                  ...
                  {baselineProjection?.yearByYear?.filter(p => p.liquidations?.some(l => l.type === 'release')).map((p, idx) => (
                    <ReferenceLine
                      key={`baseline-release-${p.age}-${idx}`}
                      x={p.age}
                      stroke="#22d3ee"
                      strokeDasharray="4 4"
                      strokeOpacity={0.3}
                  ...
                  {baselineProjection?.yearByYear?.filter(p => p.loanPayoffs?.length > 0).map((p, idx) => (
                    <ReferenceLine
                      key={`baseline-loan-${p.age}-${idx}`}
                      x={p.age}
                      stroke="#c084fc"
                      strokeDasharray="4 4"
                      strokeOpacity={0.3}
                  ...
                  {/* Scenario event lines (full color) */}
                  {scenarioProjection?.yearByYear?.filter(p => p.goalsFunded?.length > 0).map((p, idx) => (
                    <ReferenceLine
                      key={`scenario-goal-${p.age}-${idx}`}
                      x={p.age}
                      stroke="#60a5fa"
                      strokeDasharray="6 4"
                      strokeOpacity={0.7}
                  ...
                  {scenarioProjection?.yearByYear?.filter(p => p.debtPayoffs?.length > 0).map((p, idx) => (
                    <ReferenceLine
                      key={`scenario-debt-${p.age}-${idx}`}
                      x={p.age}
                      stroke="#10b981"
                      strokeDasharray="5 5"
                      strokeOpacity={0.7}
                  ...
                  {scenarioProjection?.yearByYear?.filter(p => p.liquidations?.some(l => l.type === 'release')).map((p, idx) => (
                    <ReferenceLine
                      key={`scenario-release-${p.age}-${idx}`}
                      x={p.age}
                      stroke="#22d3ee"
                      strokeDasharray="4 4"
                      strokeOpacity={0.7}
                  ...
                  {scenarioProjection?.yearByYear?.filter(p => p.loanPayoffs?.length > 0).map((p, idx) => (
                    <ReferenceLine
                      key={`scenario-loan-${p.age}-${idx}`}
                      x={p.age}
                      stroke="#c084fc"
                      strokeDasharray="4 4"
                      strokeOpacity={0.7}
